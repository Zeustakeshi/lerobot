...
from .bert import BertModelWarper, generate_masks_with_special_tokens
from .configuration_upstream import TextEncoderConfig
...
class TurboVLATextEncoder(nn.Module):
    ...
    def encode_bert_hidden(self, instructions: Sequence[str], device: torch.device):
        ...
        if output_length is None:
            raise ValueError(
                "text.padding_length is required when instruction-specific lengths are configured"
            )
        ...
        self_attention = (
            torch.eye(output_length, device=device, dtype=torch.bool).expand(len(normalized), -1, -1).clone()
        )
        for group_length, indices in grouped_indices.items():
            if group_length > output_length:
                raise ValueError(
                    f"instruction padding length {group_length} exceeds output length {output_length}"
                )
